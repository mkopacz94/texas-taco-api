        ...
        IEmailNotificationsRepository _emailNotificationsRepository) : IEmailVerificationService
    {
        public async Task EnqueueVerificationEmail(Account account)
        {
            ...
            var emailNotification = new EmailNotification(
                ...
                CreateVerificationEmailBody(verificationToken),
        ...
        private static string CreateVerificationEmailBody(VerificationToken token)
        {
            return $@"
                <div style='font-family: 'Helvetica';'>
                    ...
                    <a href='{BuildVerificationUrl(token)}'><button style='padding: 0.8em 1.5em; border-radius: 50px; border: 0px; color: white; cursor: pointer; background: #ffa436;'>Verify Now</button></a>
                    <p style='font-size: 0.8em; margin-top: 3em;'>Ignore this email if you received it without signing up.</p>
                </div>";
        }

        private static string BuildVerificationUrl(VerificationToken token)
        {
            return $"http://localhost:5173/verify?token={token.Token}";
        }
    }
}
